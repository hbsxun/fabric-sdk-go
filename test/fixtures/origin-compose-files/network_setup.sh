...
function usage () {
    ...
    echo "		up       Launch the network"
    echo "		down     teardown the network"
...
function networkUp () {
    ...
    if [ "$COUCHDB" == "y" -o "$COUCHDB" == "Y" ]; then
        ENABLE_TLS=$SECURITY CHANNEL_NAME=$CH_NAME docker-compose -f $COMPOSE_FILE -f docker-compose-couch.yaml up
    else
        ENABLE_TLS=$SECURITY CHANNEL_NAME=$CH_NAME docker-compose -f $COMPOSE_FILE up
    fi
