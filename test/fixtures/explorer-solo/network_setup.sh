...
function networkUp () {
    ...
    if [ "$COUCHDB" == "y" -o "$COUCHDB" == "Y" ]; then
       ...
       ENABLE_TLS=$SECURITY CHANNEL_NAME=$CH_NAME TIMEOUT=$CLI_TIMEOUT docker-compose -f $COMPOSE_FILE -f docker-compose-couch.yaml up 
    else
       #ENABLE_TLS=$SECURITY CHANNEL_NAME=$CH_NAME TIMEOUT=$CLI_TIMEOUT docker-compose -f $COMPOSE_FILE up -d 2>&1
       ENABLE_TLS=$SECURITY CHANNEL_NAME=$CH_NAME TIMEOUT=$CLI_TIMEOUT docker-compose -f $COMPOSE_FILE up 
    fi
